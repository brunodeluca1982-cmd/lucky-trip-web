...
// ── POST /api/stripe/admin/setup-webhook ────────────────────────────────────
// One-shot: create live webhook from within the server process (has valid key).
// Returns the webhook secret so it can be stored in STRIPE_WEBHOOK_SECRET.
// Protected by ADMIN_SECRET env var or disabled in production.

router.post("/admin/setup-webhook", async (req, res) => {
  ...
  try {
    ...
    if (found) {
      return res.json({
        message: "Webhook already exists (cannot retrieve secret — need to delete+recreate to get it)",
        id:      found.id,
        url:     found.url,
        status:  found.status,
      });
    ...
    logger.info({ webhookId: webhook.id, url: webhook.url }, "Stripe live webhook created");

    res.json({
      message:        "Webhook created — save the secret below as STRIPE_WEBHOOK_SECRET",
      id:             webhook.id,
      url:            webhook.url,
      webhook_secret: webhook.secret,
    });
  } catch (err: any) {
    logger.error({ err }, "setup-webhook failed");
